...
export const defaultPrivacySettings: PrivacySetting[] = [
...
export function runPrivacyCheck(config: ProfileStudioConfig): PrivacyCheck[] {
...
export function hideSensitiveSettings(config: ProfileStudioConfig): ProfileStudioConfig {
  return {
    ...config,
    privacy: config.privacy.map((setting) =>
      setting.sensitive ? { ...setting, visibleInReadme: false, visibleInPages: false } : setting
    )
  };
}
